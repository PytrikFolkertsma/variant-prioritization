...
def drop_na_rows(df):
	...
	df = df.dropna(subset=['RawScore', 'ReMM_score', 'DeepSEA_Functional_significance_score', 'Eigen_raw', 'Eigen PC_raw', 'LINSIGHT_score'], how='all')
	return df
...
def run_preprocessing(path, convert_categorical_vars=True, drop_rows=True):
	...
	if convert_categorical_vars:
		dummies = get_categorical_dummy_df(df, num_vars=5)
	# 	if is_testset:
...
def main():
	...
	dataset = run_preprocessing(sys.argv[1], convert_categorical_vars=True, drop_rows=False)
	print('Saving dataset')
	dataset.to_csv(sys.argv[1][:-4] + '_preprocessed.txt', sep='\t', index=False)
