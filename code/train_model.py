...
def main():
	...
	print_dataset_description(train)
	...
	X_train, X_val, y_train, y_val = train_test_split(train[features], train['label'], test_size=0.05, random_state=53, stratify=train['label'])

	X_test = test[features]
	y_test = test['label']

	print(sum(y_train == 'Pathogenic'), 'pathogenic variants in train set.')
	print(sum(y_val == 'Pathogenic'), 'pathogenic variants in val set.')
	...
	print('Average precision', ap)
				...
				# print('saving model')
				# pickle.dump(model, open("../model/test_model.pickle.dat", "wb"))
